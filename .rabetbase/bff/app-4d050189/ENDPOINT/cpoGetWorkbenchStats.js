 ...
 *
 * [脚本描述] 合并当前用户统计与 oaDemoCpoWorkbenchDashboard 自定义 SQL 聚合结果；
 ...
 */
const DASHBOARD_SQL_NAME = "oaDemoCpoWorkbenchDashboard";
...
async function countRunningFlows(models) {
  // 平台流运行中数量 = 各流程绑定主表 flow_status=SUBMITTED 之和。
  // legacy biz_task 已废弃清空，不再作为待办口径。
  const flowBoundTables = [
    "expense_application",
    "payment_application",
    "contract_application",
    "travel_application",
    "salary_payment_application",
    "invoice_application",
    "crm_contract",
  ];
  let count = 0;
  for (const tableName of flowBoundTables) {
    try {
...
export default async function cpoGetWorkbenchStats(params, context) {
  ...
  const [
    ...
    models.byTable("salary_payment_application").filter({
      where: applicantWhere,
      ...one,
    }),
    ...
    models.byTable("travel_application").filter({
      where: applicantWhere,
      ...one,
    }),
    models.byTable("payment_application").filter({
    ...
    models.byTable("company_credential").filter({
      where: { status: { $eq: "expiring" } },
      ...one,
    }),
    context.client.sql.byName(DASHBOARD_SQL_NAME).execute({ params: {} }),
  ]);
  const runningApprovalCount = await countRunningFlows(models);
